...
final class InitialStartTitleCell: PreparableTableCell {

    private let titleLabel = with(UILabel()) {
        $0.apply(.cellTitle)
    }

    var props: InitialStartTitleCell.Props = .default {
    ...
    override func prepare(withViewModel viewModel: PreparableViewModel?) {
        guard let model = viewModel as? InitialStartTitleCellViewModel else {
            return
        }
...
extension InitialStartTitleCell {
